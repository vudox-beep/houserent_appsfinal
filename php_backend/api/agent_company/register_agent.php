<?php
/**
 * Agent registration (separate from landlord/dealer register.php).
 * POST /php_backend/api/agent_company/register_agent.php
 *
 * Creates users.role = 'agent' + row in `agents` table.
 * Same dealer-style panel in the app; K20 monthly (not company).
 */
ob_start();
...
try {
    ...
    echo json_encode([
        'status' => 'error',
        'message' => 'Agent registration unavailable.',
        'error' => $e->getMessage(),
    ]);
    exit;
}
...
if ($limiter->isBlocked($ip . '_register_agent')) {
...
try {
    ...
    if ($exists->fetchColumn()) {
        $limiter->record($ip . '_register_agent');
    ...
    $subStatus = $trial['enabled'] ? 'active' : 'inactive';
    $subExpiry = $trial['enabled']
        ? date('Y-m-d H:i:s', strtotime('+' . $trial['days'] . ' days'))
        : null;

    $agentStmt = $conn->prepare(
        'INSERT INTO agents (user_id, subscription_status, subscription_expiry)
    ...
    if ($emailSent) {
        hr_json_success(
            'Agent registration successful! Please check your email to verify your account.',
            $payload
        );
    }

    hr_json_success(
        'Agent registration successful, but failed to send email. Use resend verification on the login screen.',
        $payload
    );
} catch (Throwable $e) {
